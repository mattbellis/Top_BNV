...
for i in range(nentries):
    ...
    if len(topCandidatesH) >= 1:
        ...
        for e in range(nelectrons):
            ...
            isovarE = (iso[e]+isoH[e]+isoE[e])/elecptF
            isovarB = (iso[e]+isoH[e]+max(0.,isoE[e]-1))/elecptF
...
plt.figure()
lch.hist_err(Hmasses, bins = 100, color = 'red', label = 'Hadronic')
lch.hist_err(Emasses, bins = 100, color = 'blue', label = 'Semileptonic Muon')
lch.hist_err(Mmasses, bins = 100, color = 'black', label = 'Semileptonic Electron')
plt.legend()
plt.show()
